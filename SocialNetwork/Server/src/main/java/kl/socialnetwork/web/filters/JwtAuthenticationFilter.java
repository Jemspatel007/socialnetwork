...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        try {
            UserLoginBindingModel loginBindingModel = mapper.readValue(request.getInputStream(), UserLoginBindingModel.class);
            ...
            // Log the exception to understand the exact cause of authentication failure
            System.err.println("Authentication failed: " + ex.getMessage());
            throw ex;
        } catch (IOException ex) {
            ex.printStackTrace();
            throw new CustomException("Invalid login request");
        }
    }
